...
class AES
{
    ...
    public static function aes256Encrypt($key, $data)
    {
        ...
        $padding = 16 - (strlen($data) % 16);
        $data .= str_repeat(chr($padding), $padding);
        return base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $key, $data, MCRYPT_MODE_CBC, str_repeat("\0", 16)));
    }
    ...
    public static function aes256Decrypt($key, $data)
    {
        ...
        if (32 !== strlen($key))
            $key = hash('SHA256', $key, true);
        $data = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $key, $data, MCRYPT_MODE_CBC, str_repeat("\0", 16));
        $padding = ord($data[strlen($data) - 1]);
        return substr($data, 0, -$padding);
    }
