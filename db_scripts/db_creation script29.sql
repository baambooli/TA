...
-- Script date 1/10/2013 11:12:17 AM
...
SELECT `clients`.`Id` AS `ClientId`
     , concat(`clients`.`Name`, ' ', `clients`.`Family`, ', username = ', convert(`users`.`username` USING utf8)) AS `FullName`
FROM
  (`clients`
JOIN `users`
...
SELECT `rooms`.`Id` AS `RoomId`
     ...
     , `hotels`.`Category` AS `HotelCategory`
     , `roomtypes`.`PricePerDay` AS `PricePerDay`
     , `hotels`.`Tel` AS `HotelTel`
FROM
  (((`rooms`
JOIN `hotels`
...
SELECT `rooms`.`Id` AS `RoomId`
     ...
     , `roomtypes`.`Name` AS `RoomType`
     , `roomtypes`.`PricePerDay` AS `PricePerDay`
     , `room_clients`.`StartDate` AS `CheckinDate`
     , `room_clients`.`EndDate` AS `CheckoutDate`
     , `room_clients`.`Status` AS `RoomStatus`
     , `hotels`.`Address` AS `HotelAddress`
     , `hotels`.`Image` AS `HotelImage`
     , `roomtypes`.`Capacity` AS `RoomCapacity`
...
SELECT `hotels`.`Name` AS `HotelName`
     , `hotels`.`Category` AS `HotelCategory`
     , `hotels`.`Tel` AS `HotelTel`
     , `hotels`.`Fax` AS `HotelFax`
     , `hotels`.`Address` AS `HotelAddress`
     , `hotels`.`Email` AS `HotelEmail`
     , `hotels`.`Image` AS `HotelImage`
     , `rooms`.`RoomNumber` AS `RoomNumber`
     , `rooms`.`Tell` AS `RoomTel`
     , `room_clients`.`Id` AS `RoomClientId`
     , `room_clients`.`StartDate` AS `StartDate`
     , `room_clients`.`EndDate` AS `EndDate`
     , `room_clients`.`Status` AS `Status`
     , `clients`.`Name` AS `ClientName`
     , `clients`.`Id` AS `ClientId`
     , `clients`.`Family` AS `ClientFamily`
     , `clients`.`BirthDay` AS `ClientBirthDay`
     , `clients`.`Address` AS `ClientAddress`
     , `clients`.`tell` AS `ClientTel`
