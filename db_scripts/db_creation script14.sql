...
-- Script date 12/23/2012 5:04:23 PM
-- Server version: 5.5.16
-- Client version: 4.1

--
-- Definition for database travelagency
--
...
--
-- Definition for table operator_group
--
...
--
-- Definition for table flight_passengers
--
CREATE TABLE IF NOT EXISTS flight_passengers(
  ...
  PassportExpirey DATE DEFAULT NULL,
  ...
  CONSTRAINT FK_flight_passengers2 FOREIGN KEY (CountryId)
  REFERENCES countries (Id) ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = INNODB
CHARACTER SET utf8
...
--
-- Definition for view client_view
--
CREATE OR REPLACE
DEFINER = 'root'@'localhost'
VIEW client_view
...
SELECT `clients`.`Id` AS `Id`
     ...
     , `clients`.`RoomId` AS `RoomId`
     , `room_clients`.`OccupationDate` AS `OccupationDate`
FROM
  (`room_clients`
JOIN `clients`
