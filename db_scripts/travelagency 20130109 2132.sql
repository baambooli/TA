...
-- Script date 1/9/2013 9:34:02 PM
...
--
-- Definition for table website_operators
--
DROP TABLE IF EXISTS website_operators;
...
--
-- Definition for table website_users
--
...
CREATE TABLE flights (
  ...
  CONSTRAINT fk_flights_airlines FOREIGN KEY (AirlineId)
    REFERENCES airlines(Id) ON DELETE CASCADE ON UPDATE CASCADE,
...
VIEW airplane_view
AS
	select `airplanes`.`Id` AS `Id`,`airplanes`.`Name` AS `AirplaneName`,`airlines`.`Name` AS `AirlineName`,`airplanes`.`StartDateOfWork` AS `StartDateOfWork` from (`airplanes` join `airlines` on((`airplanes`.`AirlineId` = `airlines`.`Id`)));
...
--
-- Table travelagency.reservations does not contain any data (it is empty)
...
-- 
-- Dumping data for table website_operators
--
-- Table travelagency.website_operators does not contain any data (it is empty)

-- 
-- Dumping data for table website_users
--
-- Table travelagency.website_users does not contain any data (it is empty)
...
--
-- Table travelagency.flights does not contain any data (it is empty)
...
--
-- Table travelagency.flight_clients does not contain any data (it is empty)
